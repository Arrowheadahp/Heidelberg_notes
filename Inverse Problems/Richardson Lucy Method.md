This is an iterative approach that is done by Expectation Maximisation or [[Maximum Likelihood]] of the [[Poisson Distribution]]. 
The iteration is $$f_j^{new} = \frac{f_j^{old}}{\sum_{i=1}^nA_{ij}}\sum_{i=1}^mA_{ij}\frac{g_{i}}{\sum_{k=1}^nA_{ik}f_k^{old}} $$
