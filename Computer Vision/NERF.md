...
- The [[Neural Network]] maps from 3D location and view direction to colour and density
- We train one [[Neural Network]] for all 3D locations of a static scene.
- Density $\sigma$ describes how solid/transparent a 3D point is (ex. fog)
...
c = colours
How much light is let through earlier along the ray: T = Weights$$ T_i = \prod_{j=1}^{i-1}(1-\alpha_j)$$
...
3. predict colour and density with F
...
View dependency can be visualized by querying colour of the same pixels for different viewing angles.
