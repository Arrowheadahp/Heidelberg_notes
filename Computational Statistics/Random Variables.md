Let X be a random Variable that follows the probability distribution $P(X)=f(x)$. It is the Probability Distribution Function (PDF) and Cumulative Distribution Function (CDF) is $$CDF\ F(y) := P(X\le y) = \int_{-\infty}^y f(x) dx $$
### [[Expected Values]]
