...
- Transition Matrix: $P_\pi(s'| s)\gets\sum_a\pi(a|s)p(s'|s,a)$
So the equation becomes $$\nabla v_\pi(s)=\varphi(s)+\gamma\sum_{s'}P_\pi(s'|s)\nabla v_\pi(s') $$
Unrolling for $\nabla v_\pi(s')$ K times:
$$\nabla v_\pi(s) = \sum_{t=0}^{K-1}\left(\gamma^t\sum_uPr_\pi(s\to u\ in\ t\ steps)\varphi(u) + \gamma^K\sum_uPr_\pi(s\to u,K)\nabla v_\pi(u) \right)$$
At $T=\infty$,
