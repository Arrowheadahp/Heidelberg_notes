...
- We start with a random values for $v_0(s)$ and then repeatedly apply the Bellman expectation update. This is called [[Bootstrapping]]. 
	- Provided the $v_k$, we get the [[State Value Function]] for the next step using $$v_{k+1}(s) \gets \sum_a \left(\pi(a|s) \sum_{s',r}p(s',r|s,a)\left[r+\gamma v_k(s')\right]\right)$$
